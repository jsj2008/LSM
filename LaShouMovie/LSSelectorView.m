...
@implementation LSSelectorView

@synthesize selectorViewType=_selectorViewType;
...
{
    if(![_positionArray isEqual:positionArray])
    {
        if(_positionArray!=nil)
        {
            LSRELEASE(_positionArray)
        }
        _positionArray=[positionArray retain];
        if(_positionArray!=nil)
        {
            [_tableView reloadData];
            [_tableView scrollToRowAtIndexPath:[NSIndexPath indexPathForRow:_selectIndex inSection:0] atScrollPosition:UITableViewScrollPositionMiddle animated:NO];
        }
    }
}
...
- (void)setContentFrame:(CGRect)contentFrame
{
    ...
    _tableView.frame=_contentFrame;
}

- (void)setSelectorViewType:(LSSelectorViewType)selectorViewType
{
    _selectorViewType = selectorViewType;
